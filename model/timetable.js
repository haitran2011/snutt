...
TimetableSchema.statics.update_lecture = function (timetable_id, lecture_id, lecture_raw, next) {
    ...
    (function (timetable_id, lecture_id, lecture, patch) {
        mongoose.model("Timetable").findOneAndUpdate({ "_id": timetable_id, "lecture_list._id": lecture_id }, { $set: patch }, { new: true }, function (err, doc) {
            if (err)
                return next(err);
            if (!doc)
                err = new Error("timetable not found");
            else if (!doc.lecture_list.id(lecture_id))
...
TimetableSchema.methods.delete_lecture = function (lecture_id, callback) {
    return mongoose.model("Timetable").findOneAndUpdate({ '_id': this._id }, { $pull: { lecture_list: { _id: lecture_id } } }, { new: true })
        .exec(callback);
};
