...
router.post('/password', function (req, res, next) {
  var user:UserDocument = req["user"];
  if (user.credential.local_id) return res.status(403).json({message: "already have local id"});
...
router.put('/password', function (req, res, next) {
  var user:UserDocument = req["user"];
  if (!user.credential.local_id) return res.status(403).json({message: "no local id"});
...
router.post('/facebook', function (req, res, next) {
  var user:UserDocument = req["user"];
  if (user.credential.fb_id) return res.status(403).json({message: "already attached"});
  ...
  auth.fb_auth(req.body.fb_id, req.body.fb_token, function(err, _, info) {
    ...
    UserModel.get_fb(info.fb_name, info.fb_id, function(err, result) {
      ...
      if (result) return res.status(403).json({message: "already attached with this fb_id"});
...
router.delete('/facebook', function (req, res, next) {
  var user:UserDocument = req["user"];
  if (!user.credential.fb_id) return res.status(403).json({message: "not attached yet"});
  if (!user.credential.local_id) return res.status(403).json({message: "no local id"});
